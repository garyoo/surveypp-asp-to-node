...
export class TableMulti extends SurveyModule{
    ...
    render(): JQuery<HTMLDivElement> {
        ...
        let matrixWidth = 40;
        let $col:JQuery<HTMLDivElement> = $(`<div class="d-table-cell border"></div>`);
        $col.appendTo($row);
        $col.css({'width':`${matrixWidth}%`});
        for(let ex of this.measure) {
            $col = $(`<div class="d-none d-sm-table-cell border text-center align-middle bg-secondary font-weight-bold p-2" style="width:${(100-matrixWidth)/this.measure.length}%;">${this.htmlEntitiesDecode(ex.ExampleText)}</div>`);
            $col.appendTo($row);
        }

        let idx = 0;
        for(let ex of this.matrix) {
            let rowName = `${this.que.QtnName}_${ex.ExampleVal-100}`;
            $row = $(`<div class="d-block d-sm-table-row mb-4 mb-sm-0 w-100" data-row="${ex.ExampleVal}" data-idx="${idx}" data-name="${this.que.QtnName}_${ex.ExampleVal-100}"></div>`);
            $row.appendTo($tableDiv);
            ...
            $col.appendTo($row);
            let $matrixLabel = $(`<span class="d-none d-sm-block px-1 py-2">${this.htmlEntitiesDecode(ex.ExampleText)}</span>`);
            ...
            //폼 체크 강제로 할 수 있도록..
            //MARK: 다중 체크 이므로.
            this.validForms[rowName] = {flag: true, group: new Set(this.measure.map(m => `${rowName}_${m.ExampleVal}`))};
            for(let ex2 of this.measure) {
                let formID = `${rowName}_${ex2.ExampleVal}`;
                $col = $(`<div class="d-block d-sm-table-cell border text-center p-0 m-0 align-middle example-cols-div"></div>`);
                $col.appendTo($row);
                let $label: JQuery<HTMLLabelElement> = $(`<label class="w-100 h-100 align-middle text-left text-sm-center m-0 p-2 p-sm-0 example-label example-label-matrix" for="${formID}"></label>`);
                $label.appendTo($col);
                let $checkbox: JQuery<HTMLInputElement> = $(`<input type="checkbox" id="${formID}" name="${formID}" value = "${ex2.ExampleVal}" data-idx="${idx}" data-name="${rowName}"/>`) ;
                this.$forms[formID] = {form: $checkbox[0], name: `${rowName}`, etc: false};
