...
/*
    ...
    TODO: 해당 모듈은 정보 확인해서 수정 필요

 */

export class RandomQuestion extends SurveyModule{
    ...
    render(): JQuery<HTMLDivElement> {
        ...
        //선택 가능한 보기가 1개라면.
        let cnt = this.examples.filter(q => q.Show);

        if (cnt.length === 1) {
            this.$forms[`${this.que.QtnName}_${cnt[0].ExampleVal}`].form.checked = true;
            //$div.addClass('d-none');
